...
2. Обери вільну адресу поза знайденим пулом і поза VPN-підмережею (`10.10.0.0/24`), переконайся,
   ...
   ```
3. Виключи `<LXC_IP>` з DHCP-пулу, розбивши його діапазон навколо обраної адреси (приклад:
   пул був `10.10.5.10-10.10.5.200`, резервуємо `10.10.5.50`):
   ```
   /ip pool set [find name=<назва-пулу>] ranges=10.10.5.10-10.10.5.49,10.10.5.51-10.10.5.200
   ```
4. Саме цей IP іде в пп. 1.2, 1.3 і 2.2.

**Rollback фази 0:** нічого не змінювалось (резервацію пулу з п. 0.4.3 за потреби поверни
`/ip pool set [find name=<назва-пулу>] ranges=<оригінальний-діапазон>`).
