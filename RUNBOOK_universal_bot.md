...
Скрипт запускається **на Proxmox-хості** й робить усю рутину оновлення за один прохід. Завантажити (одноразово):
```
...
Налаштування — змінні оточення (`CTID`, `APP_DIR`, `SERVICE`, `REPO`, `BRANCH`) або файл `deploy.local.conf` поруч зі скриптом (він у `.gitignore`).

**Що робить по кроках:**
1. **Передпольотні перевірки** — чи є `pct`, чи запущено контейнер, чи на місці `$APP_DIR`, `venv` і `tgbot.env`.
2. **Різниця перед записом** — `diff -u` по кожному файлу коду; що збігається, те не чіпає.
3. **Захист конфігів** — `targets.json`/`access.json`/`groups.json` **ніколи не перезаписуються** (там реальні дані, яких нема в публічному репо). Скрипт лише повідомляє, чи файл усе ще збігається з шаблоном (тобто реальні дані ще не внесені), і пропонує встановити шаблон, **тільки якщо файлу в контейнері немає** — тобто на першому деплої. `tgbot.env` не чіпається взагалі.
4. **Бекап** — копія попередніх версій у `$APP_DIR/.backups/<дата-час>/` перед записом.
5. **Перевірка синтаксису** — `py_compile` у venv контейнера + валідність JSON усіх конфігів. Не пройшло → рестарту не буде.
6. **Підтвердження рестарту** — окремим питанням, після файлів.
7. **Перевірка живучості** — після рестарту стежить `HEALTH_WAIT` секунд (за замовч. 8), що `MainPID` не змінюється: кривий конфіг дає `exit 2` + `Restart=always`, тож перші секунди виглядають нормально, і саме цикл перезапусків тут і ловиться.
8. **Автовідкат** — якщо сервіс не піднявся, показує журнал і пропонує повернути файли з бекапу та перезапустити.
