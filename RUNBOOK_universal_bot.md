...
**Файли (5 + 1 опційний):** `bot.py` (двигун + дії), `config.py` (env + валідація), `targets.json` (цілі), `access.json` (права), `tgbot.env` (секрети), `groups.json` (опційно — ролі прав, групи цілей, групи сповіщень).
...
Після `/start` користувач отримує **постійну клавіатуру** під полем вводу — по кнопці на кожну доступну саме йому дію (📊 Статус, 🔴 Заблокувати, 🟢 Розблокувати, ⚡ Розірвати сесії, ⛔/✅ WireGuard). Команд знати не треба: черговий оператор із єдиним правом `"block:ccm-sales"` бачить рівно одну кнопку — натиснув → підтвердив → готово.

Кнопки й меню будуються з того ж реєстру `ACTIONS` і прав користувача, тож нова дія автоматично отримує свою кнопку (поле `btn` у записі `ACTIONS`), а зміна прав у `access.json` міняє набір кнопок після `/start`.

Клавіатура оновлюється лише при `/start` — після зміни прав попроси користувача натиснути `/start` ще раз.
...
Файл повністю опційний — відсутній `groups.json` означає «групи не використовуються», нічого не ламається. Потрібен, коли користувачів/цілей стає більше, ніж зручно тримати плоским списком.
...
```json
{
  ...
  "target_groups": {
    "sales-dept": ["ccm-sales"]
  },
  "notify_groups": {
    "sales-notify": [8905227916]
  }
}
```

- **`roles`** — перевикористовувані набори дій. У `access.json` замість дублювання `"actions"` для кожного оператора з однаковими правами: `"222222222": { "name": "Duty", "role": "duty" }`. `role` і `actions` можна поєднувати — об'єднуються.
- **`target_groups`** — іменована група цілей. Скоуп дії може посилатись на групу замість конкретної цілі: `"block:sales-dept"` дає право на **всі** цілі групи одразу; додавання нової цілі в групу пізніше автоматично поширює вже видані права, без правки `access.json`.
- **`notify_groups`** — хто отримує сповіщення (нижче). Список Telegram id, як і `access.json`, але без прив'язки до прав — сюди можна додати спостерігачів без жодних дій у боті.

Одруківка в ролі/групі, чи посилання на неіснуючу ціль/групу — так само валить старт зі списком помилок, як і в `targets.json`/`access.json`.

### Сповіщення про `/block`

Ціль у `targets.json` може мати опційне поле `"notify"` з іменем групи з `notify_groups`:
```json
"ccm-sales": { "address": "192.168.72.27", "rule": "EMERGENCY-BLOCK-SRV", "notify": "sales-notify" }
```
Коли хтось успішно виконує `/block` на цій цілі — усім id із групи `sales-notify` (крім самого виконавця, якщо він і сам там є) бот шле окреме повідомлення з іменем виконавця, дією й результатом. Незалежно від того, чи має отримувач права керувати цією ціллю сам — сповіщення суто інформаційне. Збій надсилання одному отримувачу (заблокував бота, невалідний id) не зупиняє розсилку решті — лише пишеться в журнал.

> Реальні Telegram id в `notify_groups` — те саме застереження, що й для `access.json`: у публічному репо лишається лише приклад-заглушка, реальні id редагуються прямо на LXC, у git не комітяться.
...
### 3.2 Файли бота (4 шт. + `groups.json` опційно)

> 💡 **Для оновлень (не першого деплою) є `deploy.sh`** — див. розділ «Оновлення через deploy.sh» нижче. Він робить усе з цього пункту сам: показує різницю, питає підтвердження, бекапить, перевіряє синтаксис і відкочується, якщо сервіс не піднявся. Нижче — ручний варіант / перший деплой.
...
```
Якщо плануєш ролі/групи цілей/сповіщення (розділ «Групи» вище) — додатково:
```
pct push 200 /root/groups.json /opt/tgbot/groups.json
```
Заповни `targets.json` (цілі → їхні правила з п. 1.4), `access.json` (id → дії) і, за потреби, `groups.json` (формати — на початку runbook і в docstring `config.py`). Схованих секретів у JSON немає, але id адміністраторів теж не для чужих очей:
```
chmod 600 /opt/tgbot/access.json
chmod 600 /opt/tgbot/groups.json 2>/dev/null || true
```
...
# GROUPS_FILE=/opt/tgbot/groups.json
```
```
...
## Оновлення через deploy.sh

Скрипт робить усю рутину оновлення за один прохід. Завантажити (одноразово):
```
curl -fsSL https://raw.githubusercontent.com/tusechkin/telegrambot_MT_control/main/deploy.sh -o /root/deploy.sh
chmod +x /root/deploy.sh
```

### Два режими

Скрипт не прив'язаний до LXC — контейнер лише один із варіантів розгортання:

| Режим | Коли | Запуск |
|---|---|---|
| **local** | бот на цій же машині (звичайний сервер, VM, або зсередини самого контейнера) | `/root/deploy.sh` |
| **lxc** | бот у LXC, скрипт на Proxmox-хості (керує через `pct`) | `/root/deploy.sh --ctid 130` |

Режим визначається автоматично: заданий `CTID` (прапорцем `--ctid` чи змінною) → `lxc`, інакше → `local`. Щоб не тримати `--ctid` в пам'яті, поклади його в `deploy.local.conf` поруч зі скриптом (файл у `.gitignore`):
```
echo 'CTID=130' > /root/deploy.local.conf
```
Якщо запустити без `--ctid` на Proxmox-хості (де `/opt/tgbot` немає, бо бот у контейнері) — скрипт зупиниться з підказкою вказати `--ctid`, а не зіпсує щось на хості.

### Прапорці
```
/root/deploy.sh                    # різниця → підтвердження → оновлення → рестарт із перевіркою
/root/deploy.sh --dry-run          # лише показати, що змінилось
/root/deploy.sh --no-restart       # оновити файли, сервіс не чіпати
/root/deploy.sh --yes              # без питань (автоматизація)
/root/deploy.sh --no-self-update   # не пропонувати оновлення самого скрипта
/root/deploy.sh --branch xyz       # з іншої гілки
```

Решта налаштувань — змінні оточення (`APP_DIR`, `SERVICE`, `REPO`, `BRANCH`, `HEALTH_WAIT`) або той же `deploy.local.conf`.

**Що робить по кроках:**
1. **Передпольотні перевірки** — права root, наявність `curl`/`tar`/`diff`; у режимі `lxc` ще й `pct` та стан контейнера; далі `$APP_DIR`, `venv`, `tgbot.env`.
2. **Самооновлення** — якщо `deploy.sh` у репозиторії відрізняється від запущеного, показує різницю й пропонує оновитись та перезапуститись із тими ж аргументами. Робиться **до** решти роботи, щоб оновлення йшло вже новою логікою деплою. Вимикається прапорцем `--no-self-update`.
3. **Різниця перед записом** — `diff -u` по кожному файлу коду; що збігається, те не чіпає.
4. **Захист конфігів** — `targets.json`/`access.json`/`groups.json` **ніколи не перезаписуються** (там реальні дані, яких нема в публічному репо). Скрипт лише повідомляє, чи файл усе ще збігається з шаблоном (тобто реальні дані ще не внесені), і пропонує встановити шаблон, **тільки якщо файлу немає** — тобто на першому деплої. `tgbot.env` не чіпається взагалі.
5. **Бекап** — копія попередніх версій у `$APP_DIR/.backups/<дата-час>/` перед записом.
6. **Перевірка синтаксису** — `py_compile` у venv + валідність JSON усіх конфігів. Не пройшло → рестарту не буде.
7. **Підтвердження рестарту** — окремим питанням, після файлів.
8. **Перевірка живучості** — після рестарту стежить `HEALTH_WAIT` секунд (за замовч. 8), що `MainPID` не змінюється: кривий конфіг дає `exit 2` + `Restart=always`, тож перші секунди виглядають нормально, і саме цикл перезапусків тут і ловиться.
9. **Автовідкат** — якщо сервіс не піднявся, показує журнал і пропонує повернути файли з бекапу та перезапустити.
...
1. Дізнатись Telegram id (@userinfobot) — або взяти з відмови бота: невідомий користувач тепер бачить власний id у повідомленні `⛔ Доступ заборонено`.
2. Додати блок в `access.json` — або `"actions": [...]` напряму, або `"role": "<роль>"` з `groups.json`, якщо підходить готова.
3. `systemctl restart tgbot` → перевірка: `/start` показує лише видані дії.

### Додати роль / групу цілей / групу сповіщень
1. Створити (чи доповнити) `groups.json` — секція `roles`, `target_groups` або `notify_groups` (схема — розділ «Групи» вище).
2. В `access.json` — `"role": "ім'я-ролі"` замість/поряд з `"actions"`; скоуп дії може посилатись на групу (`"block:sales-dept"`).
3. В `targets.json` — опційне `"notify": "ім'я-notify-групи"`, якщо треба сповіщення про `/block`.
4. `systemctl restart tgbot` — одруківка в ролі/групі валить старт із чіткою помилкою, як і скрізь.

**Код не змінюється** і тут — ролі/групи розгортаються в звичайні `дія:ціль` при завантаженні конфігу.

### Додати нову дію
1. У `bot.py` — функція-виконавець `(tname|None) -> (ok, text)` (блокуючі виклики всередині — двигун сам виконає її через `to_thread`).
2. Один запис в `ACTIONS`: ключ (= команда = ключ права), рядок меню, `per_target`, текст підтвердження, виконавець, **підпис кнопки** `btn` (має бути унікальним — за ним меню розпізнає натискання).
3. Роздати право в `access.json` → рестарт. Команда, кнопка в меню, кнопки вибору цілі, підтвердження з TTL, перевірка прав і аудит — усе підхоплюється двигуном.
